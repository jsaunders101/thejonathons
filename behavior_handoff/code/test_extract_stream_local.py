...
reproduce it EXACTLY (np.array_equal, no tolerance) on:
  ...
  5. trace_viewer_local.FrameSource on a truncated file — every frame reachable
  6. peak RSS stays bounded on a ~420 MB movie extracted with batch_mb=16
     (subprocess; catches any whole-movie materialization)

Run:  python test_extract_stream_local.py <workdir>
"""
...
from box_extract_local import extract_run, run_tifs, to_gray, truncated_memmap  # noqa: E402
...
def test_truncated(work):
    ...
    from trace_viewer_local import FrameSource
...
def test_rss(work):
    ...
    code = (
        ...
        f"from box_extract_local import extract_run\n"
