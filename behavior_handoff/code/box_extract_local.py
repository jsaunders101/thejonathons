#!/usr/bin/env python3
"""
box_extract_local.py — box-ROI drawing (lightweight GUI) + per-box trace extraction
...
Every derived file lives in a proc/ subfolder INSIDE the run folder it came from:
    ...
    <run>/proc/
        ...
        <run>_sync.json        from sync_detect_local.py
        <run>_syncqc.png       from sync_detect_local.py
...
Examples:
  python box_extract_local.py draw    /data/behavior/day1
  python box_extract_local.py draw    /data/behavior --boxes-from /data/behavior/day1/run01/proc/boxes.json
  python box_extract_local.py extract /data/behavior
  python box_extract_local.py extract /data/behavior --run-index $SLURM_ARRAY_TASK_ID
  python box_extract_local.py list    /data/behavior
  python box_extract_local.py collect /data/behavior --dest /data/behavior/_collected
"""
...
try:
    ...
    sys.exit("box_extract_local.py requires tifffile (pip install tifffile)")
